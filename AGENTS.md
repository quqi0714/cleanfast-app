...
> 本文件面向接手本仓库的 AI 编码代理（Codex / Claude / 其他）。
...
2. **所有者是基督徒**：一切公开文案（商店、官网、社媒）避免其他宗教来源
   词汇——"辟谷"（道教）、"过午不食"（佛教）等一律不用，换中性生活用语。
...
```bash
# 构建
xcodebuild build -project CleanFast-Claude.xcodeproj -scheme CleanFastClaude \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro'

# 单元测试（当前 34 个，提交前必须全绿）
xcodebuild test -project CleanFast-Claude.xcodeproj -scheme CleanFastClaude \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -only-testing:CleanFastClaudeTests
```
...
- **持久化**：App Group `group.com.MaxQ.CleanFast` 的 UserDefaults，
...
- **⚠️ MIRROR 不变量**：`FastingTimerViewModel.advanceAutomaticCycle` 与
  `CleanFastClaudeWidget/WidgetData.swift` 的 `WidgetSnapshot.advanceAutomaticCycle`
...
- Bundle ID：`com.MaxQ.CleanFast`（与法律主体无关，勿因公司名调整）
